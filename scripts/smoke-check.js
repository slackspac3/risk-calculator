...
const testPocWorkflow = read('.github/workflows/test-poc.yml');
...
const releaseBootstrapAssetMatch = releaseBootstrapJs.match(/assetVersion:\s*['"]([^'"]+)['"]/);
const releaseBootstrapBuildMatch = releaseBootstrapJs.match(/build:\s*['"]([^'"]+)['"]/);
const appBuildMatch = appJs.match(/build:\s*['"]([^'"]+)['"]/);
const releaseBootstrapVersionMatch = releaseBootstrapJs.match(/version:\s*['"]([^'"]+)['"]/);
expect(releaseBootstrapAssetMatch?.[1] === versions[0], 'releaseBootstrap.js assetVersion does not match index.html asset version');
expect(appBuildMatch?.[1] && releaseBootstrapBuildMatch?.[1] === appBuildMatch[1], 'releaseBootstrap.js build stamp does not match app.js build stamp');
expect(releaseBootstrapVersionMatch?.[1] === String(packageJson.version || '').trim(), 'releaseBootstrap.js package version does not match package.json');
...
expect(ciWorkflow.includes('validate_app:'), 'Pilot AI Quality workflow is missing the blocking app-integrity validation job');
expect(ciWorkflow.includes('npm run qa:app'), 'Pilot AI Quality workflow must run the app-integrity gate on pull requests');
expect(ciWorkflow.includes('npm run qa:ai'), 'Pilot AI Quality workflow is missing the AI-quality gate');
expect(ciWorkflow.includes('continue-on-error: true'), 'Pilot AI Quality workflow must keep the AI-quality job advisory while the baseline remains below target');
expect(ciWorkflow.includes('node-version-file: .nvmrc'), 'Pilot AI Quality workflow must use the repo Node pin.');
expect(ciWorkflow.includes('actions/checkout@v6'), 'Pilot AI Quality workflow must use actions/checkout@v6.');
expect(ciWorkflow.includes('actions/setup-node@v6'), 'Pilot AI Quality workflow must use actions/setup-node@v6.');
...
expect(testPocWorkflow.includes('contents: read'), 'Test PoC workflow must use read-only contents permission');
expect(testPocWorkflow.includes('group: test-poc-validation'), 'Test PoC workflow must not share the live Pages deployment concurrency group');
...
expect(releaseChecklist.includes('Review the separate `Pilot AI Quality` job'), 'Release checklist is missing the separate AI-quality workflow confirmation');
