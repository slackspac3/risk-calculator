...
function getRateLimitKey(req, session) {
  return `ai-assumption-register::${String(session?.username || 'anonymous').trim().toLowerCase()}::${String(req.socket?.remoteAddress || 'unknown')}`;
}
...
module.exports = async function handler(req, res) {
  ...
  const normalisedInput = normaliseAssumptionRegisterWorkflowInput(body);
  const routeName = 'assumption-register';
  const result = await withAiRouteMetrics(routeName, () => withWorkflowReuse({
    ...
    compute: () => buildAssumptionRegisterWorkflow({
      ...normalisedInput,
      session
    })
  }));
