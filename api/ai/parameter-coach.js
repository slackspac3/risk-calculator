...
function getRateLimitKey(req, session) {
  return `ai-parameter-coach::${String(session?.username || 'anonymous').trim().toLowerCase()}::${String(req.socket?.remoteAddress || 'unknown')}`;
}
...
module.exports = async function handler(req, res) {
  ...
  const normalisedInput = normaliseParameterCoachWorkflowInput(body);
  const routeName = 'parameter-coach';
  const result = await withAiRouteMetrics(routeName, () => withWorkflowReuse({
    ...
    compute: () => buildParameterCoachWorkflow({
      ...normalisedInput,
      session
    })
  }));
