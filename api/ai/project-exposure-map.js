...
function getRateLimitKey(req, session) {
  return `ai-project-exposure-map::${String(session?.username || 'anonymous').trim().toLowerCase()}::${String(req.socket?.remoteAddress || 'unknown')}`;
}
...
module.exports = async function handler(req, res) {
  ...
  const normalisedInput = normaliseProjectExposureWorkflowInput(body);
  const routeName = 'project-exposure-map';
  const result = await withAiRouteMetrics(routeName, () => withWorkflowReuse({
    ...
    compute: () => buildProjectExposureMapWorkflow({
      ...normalisedInput,
      session
    })
  }));
