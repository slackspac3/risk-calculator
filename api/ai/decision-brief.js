...
function getRateLimitKey(req, session) {
  return `ai-decision-brief::${String(session?.username || 'anonymous').trim().toLowerCase()}::${String(req.socket?.remoteAddress || 'unknown')}`;
}
...
module.exports = async function handler(req, res) {
  ...
  const normalisedInput = normaliseDecisionBriefWorkflowInput(body);
  const routeName = 'decision-brief';
  const result = await withAiRouteMetrics(routeName, () => withWorkflowReuse({
    ...
    compute: () => buildDecisionBriefWorkflow({
      ...normalisedInput,
      session
    })
  }));
